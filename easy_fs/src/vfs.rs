...
type DirEntryArray = [ShortDirEntry; 16];
...
impl VFile{
    ...
    pub fn set_first_cluster(&self, clu:u32){
        self.modify_short_dirent(|se:&mut ShortDirEntry|{se.set_first_cluster(clu)})
    }
    ...
    // 获取目录中offset处目录项的信息,返回<size, atime, mtime, ctime>
    pub fn stat(&self)->( i64, i64, i64, i64, u64 ){
        self.read_short_dirent(|sde:&ShortDirEntry|{
            ...
            if self.is_dir() {
                ...
                size = cluster_num * fs_reader.get_bytes_per_cluster();
